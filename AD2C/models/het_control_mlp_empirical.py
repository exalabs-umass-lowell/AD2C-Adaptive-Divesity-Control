...
class HetControlMlpEsc(Model):
    ...
    def _update_esc_controller(self, tensordict: TensorDictBase) -> None:
        ...
        with torch.no_grad():
            reward = tensordict.get(reward_key)  # [*batch, n_agents, ...] typically
            J = reward.mean()  # global scalar objective

            # EMA of reward
            self.s_reward.mul_(1 - self.tau).add_(J * self.tau)

            # Gradient estimate via correlation with dither
            gradient_estimate = self.s_reward * self.last_dither  # scalar broadcast
            k_hat_update = self.esc_gain * gradient_estimate

            # Update k_hat (stable scalar step)
            self.k_hat.add_(k_hat_update)

            # Log (as scalars on reward device/dtype)
            rdev, rdtype = reward.device, reward.dtype
            tensordict.set((self.agent_group, "esc_reward_J"), torch.as_tensor(self.s_reward, device=rdev, dtype=rdtype))
            tensordict.set((self.agent_group, "esc_grad_estimate"), torch.as_tensor(gradient_estimate, device=rdev, dtype=rdtype))
            tensordict.set((self.agent_group, "esc_k_hat_update"), torch.as_tensor(k_hat_update, device=rdev, dtype=rdtype))
            tensordict.set((self.agent_group, "k_hat_new"), self.k_hat.to(device=rdev, dtype=rdtype))
